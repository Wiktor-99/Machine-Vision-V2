...
void Device::getData()
{
    ...
    while(serialPort.canReadLine()) {
        ...
        if(strSplited.at(0) == 'P')
        {
         if(strSplited.at(1).toDouble() == 55)
         {
             if(sortingFlag)
             {
                 char id = 'P';
                 ...
                 { qDebug()<<"Position Reached: "<< positionReached ;

                     emit moveToBoxPosition();
                 }

                 if(positionReached == 2)
                 {
                     qDebug()<<"Position Reached: "<< positionReached ;
                    emit moveToCapPosition();
                 }
