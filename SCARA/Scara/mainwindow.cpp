...
std::tuple<int, int, char> decode_message(char* message){
    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->resetViewButton->setDisabled(1);
    ui->positionSetButton->setDisabled(1);
...
void MainWindow::addConnections()
{
   ...
   // connect(objectScene, &Object_Scene::positionChanged, objectScene, &Object_Scene::setPosition);
    ...
    connect(device,&Device::moveToCapPosition,this,&MainWindow::sendCapPositon);
    connect(device,&Device::moveToBoxPosition,this,&MainWindow::segregateCap);
    ...
    connect(ui->actionNarysuj_linie, &QAction::triggered,[=]()
    {
        ...
        while (true) {
            if (read(fd, message, message_size) > 0) {
                ...
                //qDebug() << "x: " << x <<  " y: " << y << " color: " << color << "\n";
                this->capPositionVector.push_back(std::make_tuple(x/100.0,y/100.0,color));

            }
        }

        //this->capPositionVector.push_back(std::make_tuple(7.0,14.0,'b'));
        //this->capPositionVector.push_back(std::make_tuple(-7.0,14.0,'b'));
           //qDebug()<<"jestem tu1";
        if(this->capPositionVector.size() > 0)
        {
            device->sortingFlag = true;
            //qDebug()<<"jestem tu2";
            emit device->moveToCapPosition();
        }
    });
    //connect(ui->actionNarysuj_linie, &QAction::triggered,this,&MainWindow::detectCaps);
}
       // connect(ui->menuWybierz_port, &QMenu::triggered, this, &MainWindow::selectPort);
void MainWindow::detectCaps()
{
    int fd;
    const char *fifo = "pipe";
    constexpr std::size_t message_size{20};
...
void MainWindow::sendCapPositon()
{
    device->positionReached = 0;
    int idx = this->capPositionVector.size() - 1;
    std::stringstream msg;

    if(idx>=0)
    {
    robot->fromSpinboxToPosition(std::get<0>(this->capPositionVector[idx]),std::get<1>(this->capPositionVector[idx]),4.0,ui->checkBox->isChecked());
    msg <<"K "<<(int)robot->setpoint_J1_angle << " " << (int)robot->setpoint_J2_angle << " "<< (int)robot->setpoint_J3_angle << " 55R";

    emit device->sendingMessage(QString::fromStdString(msg.str()));
    }
    else
    {
        device->sortingFlag = false;
        robot->fromSlidersToPosition(0.0,0.0,180.0);
        msg <<"K "<<(int)robot->setpoint_J1_angle << " " << (int)robot->setpoint_J2_angle << " "<< (int)robot->setpoint_J3_angle << " 55R";
        qDebug() << QString::fromStdString(msg.str());
        emit device->sendingMessage(QString::fromStdString(msg.str()));

        qDebug() << "Sortowanie zakonczone";
    }
}

void MainWindow::segregateCap()
{
    unsigned int idx = this->capPositionVector.size() - 1;
    std::stringstream msg;

    switch (std::get<2>(this->capPositionVector[idx])) {
    case 'b':
        robot->fromSlidersToPosition(185.0,85.0,180.0);
        msg <<"L "<<(int)robot->setpoint_J1_angle << " " << (int)robot->setpoint_J2_angle << " "<< (int)robot->setpoint_J3_angle << " 55R";
        this->capPositionVector.pop_back();
        emit device->sendingMessage(QString::fromStdString(msg.str()));
        break;
    case 'g':
        robot->fromSlidersToPosition(-5.0,-80.0,180.0);
        msg <<"L "<<(int)robot->setpoint_J1_angle << " " << (int)robot->setpoint_J2_angle << " "<< (int)robot->setpoint_J3_angle << " 55R";
        this->capPositionVector.pop_back();
        emit device->sendingMessage(QString::fromStdString(msg.str()));
        break;
    case 'r':
        robot->fromSlidersToPosition(180.0,0.0,180.0);
        msg <<"L "<<(int)robot->setpoint_J1_angle << " " << (int)robot->setpoint_J2_angle << " "<< (int)robot->setpoint_J3_angle << " 55R";
        this->capPositionVector.pop_back();
        emit device->sendingMessage(QString::fromStdString(msg.str()));
        break;
...
void MainWindow::on_pushButton_5_clicked()
{
    ...
    emit device->sendingMessage(QString::fromStdString(msg.str()));
    ui->resetViewButton->setDisabled(0);
    ui->positionSetButton->setDisabled(0);
